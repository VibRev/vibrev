...
`Listener::serve` takes the engine's `axum::Router` and layers the gate over *all* of it. The engine never gets to say which routes are exempt, and `AccessPolicy.auth` is not an `Option`. **There is no way to serve unauthenticated.** Checks run Origin → Host → bearer; credential failures get 401 with `WWW-Authenticate`, policy failures get 403.
